...
| LLM02 | Sensitive Information Disclosure | 4 / 4 | 0 | 1 / 4 |
...
**³ Live model.** The chat probes run against a real model (`llama3.2`, served from `http://localhost:11434/v1`) with a planted canary, measured 2026-07-22. This is a **snapshot, not a fixed result**: a live model is non-deterministic, so re-running shifts the counts by an attack or two. It is a real-world reference point, never a pass/fail control, and CI never asserts it.
