...
| OWASP | Category | Vulnerable mock ¹ | Hardened control ² | Live model<br>(llama3.2 @ http://localhost:11434/v1) ³ |
|-------|----------|:-----------------:|:------------------:|:--:|
| LLM01 | Prompt Injection | 8 / 8 | 0 | 3 / 8 |
| LLM02 | Sensitive Information Disclosure | 4 / 4 | 0 | 0 / 4 |
| LLM05 | Improper Output Handling | 3 / 3 | 0 | 0 / 3 |
| LLM06&nbsp;† | Excessive Agency | 1 / 2 | 0 | n/a |
| LLM07 | System Prompt Leakage | 4 / 4 | 0 | 1 / 4 |
...
**³ Live model.** The chat probes run against a real model (`llama3.2 @ http://localhost:11434/v1`) with a planted canary, measured 2026-07-22. This is a **snapshot, not a fixed result**: a live model is non-deterministic, so re-running shifts the counts by an attack or two. It is a real-world reference point, never a pass/fail control, and CI never asserts it.
